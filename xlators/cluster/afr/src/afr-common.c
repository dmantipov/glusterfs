...
__afr_lock_heal_synctask(xlator_t *this, afr_private_t *priv, int child)
{
    ...
    ret = synctask_new(getctx(this)->env, afr_lock_heal, afr_lock_heal_done, frame,
                       frame);
    if (ret)
...
afr_spb_choice_timeout_cancel(xlator_t *this, inode_t *inode)
{
    ...
    LOCK(&inode->lock);
    {
        ...
        ctx->spb_choice = -1;
        if (ctx->timer) {
            gf_timer_call_cancel(getctx(this), ctx->timer);
            ctx->timer = NULL;
        }
        ret = 0;
    }
...
afr_set_split_brain_choice(int ret, call_frame_t *frame, void *opaque)
{
    ...
    LOCK(&inode->lock);
    {
        ...
        if (ctx->timer) {
            if (ctx->spb_choice == -1) {
                if (!gf_timer_call_cancel(getctx(this), ctx->timer)) {
                    ctx->timer = NULL;
    ...
    reset_timer:
        ret = gf_timer_call_cancel(getctx(this), ctx->timer);
        if (ret != 0) {
    ...
    set_timer:
        ctx->timer = gf_timer_call_after(getctx(this), delta,
...
afr_txn_refresh_done(call_frame_t *frame, xlator_t *this, int err)
{
    ...
    if (ret == -EIO) {
        ...
        ret = synctask_new(getctx(this)->env, afr_fav_child_reset_sink_xattrs,
...
afr_attempt_local_discovery(xlator_t *this, int32_t child_index)
{
    ...
    newframe = create_frame(this, getctx(this)->pool);
    if (!newframe) {
        return;
    }
...

{
    ...
    ret = synctask_new(getctx(this)->env, afr_lookup_sh_metadata_wrap,
...
name_heal:
    ...
    ret = synctask_new(getctx(this)->env, afr_lookup_selfheal_wrap,
                       afr_refresh_selfheal_done, heal, frame);
    if (ret) {
        AFR_STACK_DESTROY(heal);
        goto metadata_heal;
...
afr_discover_done(call_frame_t *frame, xlator_t *this)
{
    ...
    ret = synctask_new(getctx(this)->env, afr_ta_id_file_check,
...
afr_wakeup_same_fd_delayed_op(xlator_t *this, afr_lock_t *lock, fd_t *fd)
{
    ...
    if (lock->delay_timer) {
        ...
        if (fd == local->fd) {
            if (gf_timer_call_cancel(getctx(this), lock->delay_timer)) {
                local = NULL;
            } else {
       ...
       struct gf_flock *flock, dict_t *xdata)
{
    ...
    if (xdata) {
        ...
        if (afr_is_lock_mode_mandatory(xdata)) {
            ret = synctask_new(getctx(this)->env, afr_lk_transaction,
...
__afr_launch_notify_timer(xlator_t *this, afr_private_t *priv)
{
    ...
    delay.tv_sec = 10;
    delay.tv_nsec = 0;
    priv->timer = gf_timer_call_after(getctx(this), delay, afr_notify_cbk, this);
    if (priv->timer == NULL) {
...
out:
    if (up_children == 1) {
        ...
        gf_event(EVENT_AFR_SUBVOL_UP, "client-pid=%d; subvol=%s",
                 getctx(this)->cmd_args.client_pid, this->name);
    } else {
...
afr_ta_lock_release_synctask(xlator_t *this)
{
    ...
    ret = synctask_new(getctx(this)->env, afr_release_notify_lock_for_ta,
...
afr_notify(xlator_t *this, int32_t event, void *data, void *data2)
{
    ...
    LOCK(&priv->lock);
    {
        ...
        if (!had_heard_from_all && have_heard_from_all) {
            if (priv->timer) {
                gf_timer_call_cancel(getctx(this), priv->timer);
                priv->timer = NULL;
            }
