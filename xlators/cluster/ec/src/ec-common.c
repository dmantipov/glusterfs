...
ec_lock_timer_cancel(xlator_t *xl, ec_lock_t *lock)
{
    ...
    if (gf_timer_call_cancel(getctx(xl), lock->timer) < 0) {
...
ec_unlock_timer_del(ec_lock_link_t *link)
{
    ...
    if (lock->timer != NULL) {
        ...
        gf_timer_call_cancel(getctx(link->fop->xl), lock->timer);
        lock->timer = NULL;
...
ec_lock_delay_create(ec_lock_link_t *link)
{
    ...
    delay.tv_nsec = 0;
    lock->timer = gf_timer_call_after(getctx(fop->xl), delay, ec_unlock_timer_cbk,
                                      link);
    if (lock->timer == NULL) {
