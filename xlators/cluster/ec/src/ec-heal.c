...
ec_launch_heal(ec_t *ec, ec_fop_data_t *fop)
{
    ...
    frame = create_frame(ec->xl, getctx(ec->xl)->pool);
    if (!frame) {
        ret = -1;
        goto out;
    }
    ...
    ret = synctask_new(getctx(ec->xl)->env, ec_synctask_heal_wrap, ec_heal_done,
                       frame, fop);
out:
    if (ret < 0) {
...
ec_launch_replace_heal(ec_t *ec)
{
    ...
    ret = synctask_new(getctx(ec->xl)->env, ec_replace_brick_heal_wrap,
