...
dht_discover_complete(xlator_t *this, call_frame_t *discover_frame)
{
    ...
    */
    ...
    if (source && (heal_path || layout_anomalies || error_while_marking_mds)) {
        ...
        heal_frame = create_frame(this, getctx(this)->pool);
        if (heal_frame) {
            ...
            ret = synctask_new(getctx(this)->env, dht_heal_full_path,
                         ...
                         int mark_during_fresh_lookup)
{
    ...
    */
    if (!local->xattr || !dict_get(local->xattr, conf->mds_xattr_key)) {
        ...
        */
        if (mark_during_fresh_lookup) {
            xattr_frame = create_frame(this, getctx(this)->pool);
            if (!xattr_frame) {
                ret = -1;
                goto out;
            }
...
dht_dir_xattr_heal(xlator_t *this, dht_local_t *local, int *op_errno)
{
    ...
    gf_uuid_unparse(local->gfid, gfid_local);
    copy = create_frame(this, getctx(this)->pool);
    if (copy) {
        ...
        if (!copy_local) {
            ...
            *op_errno = ENOMEM;
            ...
            ret = synctask_new(getctx(this)->env, dht_dir_heal_xattrs,
    ...
    if (op_ret == -1) {
        ...
        if (local->linked) {
            ...
            if (cleanup_frame) {
                ...
                ret = synctask_new(getctx(this)->env, dht_remove_stale_linkto,
