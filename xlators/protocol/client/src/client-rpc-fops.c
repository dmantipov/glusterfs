    ...
    if (args->xdata) {
        ...
        if (content != NULL) {
            ...
            /* This change very much depends on quick-read
               changes */
            rsp_iobuf = iobuf_get(getctx(this)->iobuf_pool);
            if (rsp_iobuf == NULL) {
                goto unwind;
            }
...
client3_3_readv(call_frame_t *frame, xlator_t *this, void *data)
{
    ...
    rsp_iobuf = iobuf_get2(getctx(this)->iobuf_pool, args->size);
    if (rsp_iobuf == NULL) {
        op_errno = ENOMEM;
        goto unwind;
    }
...
client3_3_fgetxattr(call_frame_t *frame, xlator_t *this, void *data)
{
    ...
    /* TODO: what is the size we should send ? */
    rsp_iobuf = iobuf_get2(getctx(this)->iobuf_pool, 8 * GF_UNIT_KB);
    if (rsp_iobuf == NULL) {
        op_errno = ENOMEM;
        goto unwind;
    }
