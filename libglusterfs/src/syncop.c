...
synctask_wake(struct synctask *task)
{
    ...
    {
        if (task->timer != NULL) {
            if (gf_timer_call_cancel(getctx(task->xl), task->timer) != 0) {
                goto unlock;
            }
                ...
                synctask_cbk_t cbk, call_frame_t *frame, void *opaque)
{
    ...
    newtask->frame = frame;
    if (!frame) {
        newtask->opframe = create_frame(this, getctx(this)->pool);
        if (!newtask->opframe)
            goto err;
...
synctask_timer(void *data)
{
    ...
    gf_timer_call_cancel(getctx(task->xl), task->timer);
    task->timer = NULL;
...
synctask_switchto(struct synctask *task)
{
    ...
    pthread_mutex_lock(&env->mutex);
    {
        if (task->woken) {
            ...
            if (task->delta != NULL) {
                task->timer = gf_timer_call_after(getctx(task->xl), *task->delta,
                                                  synctask_timer, task);
            }
        }
