...
default_notify(xlator_t *this, int32_t event, void *data, ...)
{
    ...
    switch (event) {
        ...
        case GF_EVENT_AUTH_FAILED: {
             ...
             * it to fuse.
             */
            if (!parent && getctx(this) && getctx(this)->root) {
                xlator_notify(getctx(this)->root, event, this->graph, NULL);
            }
            ...
            if (event == GF_EVENT_CHILD_DOWN &&
                !(getctx(this) && getctx(this)->root) && (graph->top == this)) {
        ...
        case GF_EVENT_UPCALL: {
            ...
            if (!parent && getctx(this) && getctx(this)->root)
                xlator_notify(getctx(this)->root, event, data, NULL);
